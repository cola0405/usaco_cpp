...
vector<unordered_set<int>> suf_already(5);             // record all hit for 5 offset
int main()
{
    ...
    for(int i=0; i<C; i++){
        ...
        if(c[i] == 'R') pos++;
        if(c[i] == 'F') {
            if(target[pos+100010] && cnt[pos] == 0) pre.push_back(pre.back() + 1);
    ...
    for(int i=C-1; i>=0; i--){
        if(c[i] == 'L'){
            ...
            if(target[pos+100010] && suf[1+2].find(pos) == suf[1+2].end() && cnt[pos] == 0) cnt2++;
            ans = max(ans, cnt2);
        }
        else if(c[i] == 'R'){
            pos--;
            ...
            // to F
            int cnt2 = pre[i] + suf[-1+2].size();
            // make sure pos is a target and not hit by suf and it's an available target
            if(target[pos+100010] && suf[-1+2].find(pos) == suf[-1+2].end() && cnt[pos] == 0) cnt2++;
            ans = max(ans, cnt2);
        }
        else{
            ...
            if(target[pos+100010] && cnt[pos] == 1 && suf_already[-1+2].find(pos) != suf_already[-1+2].end()) cnt1++; 
            ans = max(ans, cnt1);

            // to R
            int cnt2 = pre[i] + suf[1+2].size();
            if(target[pos+100010] && cnt[pos] == 1 && suf_already[1+2].find(pos) != suf_already[1+2].end()) cnt2++; 
            ans = max(ans, cnt2);
        }
        if(c[i] == 'F'){
            cnt[pos]--;
            for(int j=0; j<5; j++){         // build suffix to check the current position whether will be hit by offset
                int p = pos-2+j;
                if(target[p+100010] && cnt[p] == 0) suf[j].insert(p);
                suf_already[j].insert(pos);
            }

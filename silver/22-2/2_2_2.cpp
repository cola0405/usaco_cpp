...
void combination(vector<int>& inx, int start, int end, int m){    // 搜索所有可行的方案
    if(inx.size() == m){
        ...
        if(end == n){
            record2[m][curX][curY] += 1;
            if(m != (n-n/2)-m){
                record2[(n-n/2)-m][rightX-curX][rightY-curY] += 1;
            }
            
        }
        else{
            record1[m][curX][curY] += 1;
            if(m != n/2-m){     // 相同大小是能轮到的，这里就不加了
    ...
    for(int k=1; k<=n; k++){
        for(int left=0; left<=k; left++){
            ...
            for(auto it: record1[left]){
                long long lx = it.first;
                for(auto it1: record1[left][lx]){
                    long long ly = it1.first;
                    if(record2[k-left][xg-lx][yg-ly]){
                        ans[k] += it1.second * record2[k-left][xg-lx][yg-ly];
                    }
                }

...
// we have to make it as more average as possible
...
int main() {
    ...
    for(int x=b[0]; x>=1; x--){    // try all possible avg number
        ...
        for(int i=0; i<n; i++){
            int berry = b[i];
            while(berry >= x && v.size() < k){   // take ave number as more as possible
                v.push_back(x);
                berry -= x;
            }
            pq.push(berry);
        }
        while(v.size() < k){                    // fill the empty basket as more as possible
